let show=!0;const menuSection=document.querySelector(".header__navbar__section");const menuToggle=menuSection.querySelector(".header__navbar__section__toggle");const menuItem=document.querySelectorAll(".header__navbar__section__nav ul li a");menuToggle.addEventListener("click",()=>{document.body.style.overflow=show?"hidden":"initial";menuSection.classList.toggle("header__navbar--open",show);show=!show});menuItem.forEach((item)=>{item.addEventListener('click',()=>{menuSection.classList.toggle('header__navbar--open');document.body.style.overflow="initial"})});/*!
...
if(this.currentImageIndex>0){var preloadPrev=new Image();preloadPrev.src=this.album[this.currentImageIndex-1].link}};Lightbox.prototype.enableKeyboardNav=function(){this.$lightbox.on('keyup.keyboard',$.proxy(this.keyboardAction,this));this.$overlay.on('keyup.keyboard',$.proxy(this.keyboardAction,this))};Lightbox.prototype.disableKeyboardNav=function(){this.$lightbox.off('.keyboard');this.$overlay.off('.keyboard')};Lightbox.prototype.keyboardAction=function(event){var KEYCODE_ESC=27;var KEYCODE_LEFTARROW=37;var KEYCODE_RIGHTARROW=39;var keycode=event.keyCode;if(keycode===KEYCODE_ESC){event.stopPropagation();this.end()}else if(keycode===KEYCODE_LEFTARROW){if(this.currentImageIndex!==0){this.changeImage(this.currentImageIndex-1)}else if(this.options.wrapAround&&this.album.length>1){this.changeImage(this.album.length-1)}}else if(keycode===KEYCODE_RIGHTARROW){if(this.currentImageIndex!==this.album.length-1){this.changeImage(this.currentImageIndex+1)}else if(this.options.wrapAround&&this.album.length>1){this.changeImage(0)}}};Lightbox.prototype.end=function(){this.disableKeyboardNav();$(window).off('resize',this.sizeOverlay);this.$lightbox.fadeOut(this.options.fadeDuration);this.$overlay.fadeOut(this.options.fadeDuration);if(this.options.disableScrolling){$('body').removeClass('lb-disable-scrolling')}};return new Lightbox()}));const background=document.getElementById('hero');const mediaDesktop=window.matchMedia('(min-width: 1025px)');const mediaTabletPro=window.matchMedia('(max-width: 1024px)');const mediaTablet=window.matchMedia('(max-width: 768px)');const mediaPhone=window.matchMedia('(max-width: 425px)');const mediaPhoneSmall=window.matchMedia('(max-width: 375px)');function sizeMediaDesktop(mediaDesktop){if(mediaDesktop.matches){window.addEventListener('scroll',function(){background.style.backgroundSize=150- +window.pageYOffset/12+'%'})}}
sizeMediaDesktop(mediaDesktop);mediaDesktop.addListener(sizeMediaDesktop);function sizeMediaTabletPro(mediaTabletPro){if(mediaTabletPro.matches){window.addEventListener('scroll',function(){background.style.backgroundSize=280- +window.pageYOffset/12+'%'})}}
sizeMediaTabletPro(mediaTabletPro);mediaTabletPro.addListener(sizeMediaTabletPro);function sizeMediaTablet(mediaTablet){if(mediaTablet.matches){window.addEventListener('scroll',function(){background.style.backgroundSize=250- +window.pageYOffset/12+'%'})}}
sizeMediaTablet(mediaTablet);mediaTablet.addListener(sizeMediaTablet);function sizeMediaPhone(mediaPhone){if(mediaPhone.matches){window.addEventListener('scroll',function(){background.style.backgroundSize=345- +window.pageYOffset/12+'%'})}}
sizeMediaPhone(mediaPhone);mediaPhone.addListener(sizeMediaPhone);function sizeMediaPhoneSmall(mediaPhoneSmall){if(mediaPhoneSmall.matches){window.addEventListener('scroll',function(){background.style.backgroundSize=380- +window.pageYOffset/12+'%'})}}
sizeMediaPhoneSmall(mediaPhoneSmall);mediaPhoneSmall.addListener(sizeMediaPhoneSmall);const menuItens=document.querySelectorAll(['nav a[href^="#"]','#hero a[href^="#"]']);menuItens.forEach(item=>{item.addEventListener('click',scrollToIdOnClick)});function scrollToIdOnClick(){event.preventDefault();const to=getScrollTopByHref(event.target)-100;scrollToPosition(to)}
function scrollToPosition(to){smoothScrollTo(0,to,800)}
function getScrollTopByHref(element){const id=element.getAttribute('href');return document.querySelector(id).offsetTop}
function smoothScrollTo(endX,endY,duration){const startX=window.scrollX||window.pageXOffset;const startY=window.scrollY||window.pageYOffset;const distanceX=endX-startX;const distanceY=endY-startY;const startTime=new Date().getTime();duration=typeof duration!=='undefined'?duration:400;const easeInOutQuart=(time,from,distance,duration)=>{if((time/=duration/2)<1)return distance/2*time*time*time*time+from;return-distance/2*((time-=2)*time*time*time-2)+from};const timer=setInterval(()=>{const time=new Date().getTime()-startTime;const newX=easeInOutQuart(time,startX,distanceX,duration);const newY=easeInOutQuart(time,startY,distanceY,duration);if(time>=duration){clearInterval(timer)}
window.scroll(newX,newY)},1000/60)};window.addEventListener("scroll",function(){let header=document.getElementById("header");header.classList.toggle("header--active",window.scrollY>0)});window.addEventListener("scroll",function(){let whatsapp=document.getElementById("whatsapp");whatsapp.classList.toggle("whatsapp__button--active",window.scrollY>0)});/*! jQuery v3.5.1 | (c) JS Foundation and other contributors | jquery.org/license */
...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(function(){$('#form').submit(function(){$.ajax({url:'source/Support/Sender.php',type:'POST',data:$('#form').serialize(),success:function(data){$('.contact__form__status').html(data)}});return!1})});$(function(){$("#button").click(function(){$(".contact__form__status--loading").removeClass("d-none")})})